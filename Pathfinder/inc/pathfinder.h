...
struct s_isl {
	int index;
	char *name;
	t_link *links; //список всех связанных островов (вершин)
	t_path **paths;
	t_isl *next;
};

struct s_link {
	int wdth_lnk; //вес между связанными островами (вершинами)
	t_isl *linked_isl;
	t_link *next;
};
...
struct s_path {
    ...
    bool is_shortest; // истина, если дистанция этого пути минимальна в этом списке путей
	t_path *next;
};
...
void mx_pf_pop_back_link(t_link **links);
	// read file 
void mx_pf_read_file(t_main *main);// mx_parse_file
